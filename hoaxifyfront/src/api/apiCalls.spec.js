...
describe ('apiCalls', () =>{
    ...
    describe('listUsers', () => {
        ...
        it('calls /api/1.0/users?page=5&size=3 when only page params provided for listUsers', () => {
            const mockListUsers = jest.fn()
            axios.get = mockListUsers;
            apiCalls.listUsers({page: 5});
            expect(mockListUsers).toBeCalledWith('/api/1.0/users?page=5&size=3');
        })
        it('calls /api/1.0/users?page=0&size=5 when only size params provided for listUsers', () => {
            const mockListUsers = jest.fn()
            axios.get = mockListUsers;
