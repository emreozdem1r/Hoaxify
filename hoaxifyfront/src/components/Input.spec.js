...
describe('Layout', ()=> {
    ...
    it('does not display thw label when no label provied in props', ()=>{
        const { container } = render(<Input/>)
    ...
    it('has password type for input when type is not provided as prop', ()=>{
    ...
    it('has default style when there is no validation error or succes', ()=>{
        const { container } = render(<Input/>)
        const input = container.querySelector('input');
        expect(input.className).toBe('form-control')
    })
    it('has succes style when hasError property is false', ()=>{
